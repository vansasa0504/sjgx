...
| RISK-T-02 | TECHNICAL_DEBT | FR-305/804；NFR-S02 | 同一审查：归档表无 id 单列唯一索引，与测试双保险不一致 | 中 | MAY_WAIVE | 并发风险评估、迁移与目标 DB 测试或完整豁免 | DBA | 评估在线迁移并纳入 `PV-DB` 约束/索引三方比对；不得在 P3 修改 |
...
| RISK-T-06 | TECHNICAL_DEBT | NFR-S03/U01 | `reviews/claude-review-P2-04.md`：XSS 不改写 JSON body；Vite 构建警告需监控 | 中 | MAY_WAIVE | 输出编码策略/安全测试及 CI 警告基线 | 前端/安全负责人 | 纳入 PV-SEC 和 CI 跟踪 |
| RISK-T-07 | TECHNICAL_DEBT | FR-606；NFR-S02/G01 | `reviews/claude-review-P2-05-最终复查.md`：proof_hash 约束、多实例 ID、事务/verify 时机等遗留 | 中 | MAY_WAIVE | 调用链实现前关闭约束/并发/事务风险，或完整书面豁免 | 数据治理/DBA | 与 RISK-F-06 联合处理 |
