...
> 前置：P0-03 通过（JDBC 仓储可用）；P0-04 通过并提交（invoke 已改为 apiKey 服务端查 secret，提交 `f10ca229`）
> 日期：2026-06-27
...
### 本次实现
- `t_service_invoke_log` 表结构核对/补字段（使用下一迁移版本 V013 + U013，避免与 P0-04 的 V012 冲突）。
...
- **回滚**：V013 有 U013；AsyncInvokeLogWriter 保留内存实现可切回。
